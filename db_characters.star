...
def main(config):
    ...
    return get_info(api_endpoint, debug_output, show_headshot, line_one_color, line_two_color, line_three_color, line_four_color)

def get_info(api_endpoint, debug_output, show_headshot, line_one_color, line_two_color, line_three_color, line_four_color):
    character_info_dict = {
        "name": None,
        "gender": None,
        "race": None,
        ...
        if dbz_characters_dict != None:
            ...
            if get_character_string != None and type(get_character_string) == "string":
                ...
                if dbz_character_dict != None:
                    ...
                    # Base Ki
                    # Affiliation
                    child = render_character_profile(character_info_dict, character_image, planet_image, show_headshot, line_one_color, line_two_color, line_three_color, line_four_color)
                elif debug_output:
...
def render_character_profile(character_info_dict, character_image, planet_image, show_headshot = False, line_one_color = "#FFFFFF", line_two_color = "#FFFFFF", line_three_color = "#FFFFFF", line_four_color = "#FFFFFF"):
    # Show error
            ...
            render.Box(
                ...
                child = render.Padding(
                    ...
                    child = animation.Transformation(
                        ...
                        duration = duration,
                        delay = delay,
                        keyframes = getKeyframes(-64 * 2, 0, height),
                    ),
                ),
            ),
        )
    ...
    if character_info_dict["ki"] != None and len(character_info_dict["ki"]) > 0 and character_info_dict["ki"] != "unknown":
        ...
        content_array.append(
            render.Padding(
                ...
                child = animation.Transformation(
                    wait_for_child = True,
                    child = render.Text(character_info_dict["ki"] + " ki", font = "tom-thumb", color = line_three_color),
                    duration = duration,
                    delay = local_delay,
                    keyframes = getKeyframes(-64 * 2, 0, height),
                ),
...
def getKeyframes(xIn, xOut, yPos = 0):
    return [
        animation.Keyframe(
            percentage = 0.0,
            transforms = [animation.Translate(xIn, yPos)],
            curve = "ease_in_out",
        ),
        animation.Keyframe(
            percentage = 0.10,
        ...
        animation.Keyframe(
            percentage = 1.0,
            transforms = [animation.Translate(xOut, yPos)],
        ),
    ]

def get_data(url, debug_output, headerMap = {}, ttl_seconds = 60 * 60):
    if headerMap == {}:
    ...
    return schema.Schema(
        ...
        fields = [
            ...
            schema.Text(
                id = "line_one_color",
                name = "Name color",
                desc = "Name font color using Hex color codes. eg, `#FFFFFF`.",
                icon = "paintbrush",
                default = "#FFFFFF",
            ),
            schema.Text(
                id = "line_two_color",
                name = "Species/gender color",
                desc = "Species/gender font color using Hex color codes. eg, `#FFFFFF`.",
                icon = "paintbrush",
                default = "#FFFFFF",
            ),
            schema.Text(
                id = "line_three_color",
                name = "Ki color",
                desc = "Ki font color using Hex color codes. eg, `#FFFFFF`.",
                icon = "paintbrush",
                default = "#FFFFFF",
            ),
            schema.Text(
                id = "line_four_color",
                name = "Affiliation color",
                desc = "Affiliation font color using Hex color codes. eg, `#FFFFFF`.",
                icon = "paintbrush",
                default = "#FFFFFF",
            ),
        ],
